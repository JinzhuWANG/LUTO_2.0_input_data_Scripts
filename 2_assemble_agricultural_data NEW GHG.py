...
""" Note: New lmap data was provided by CSIRO (RMM) in Dec 2024 but not used because the livestock mapping is too extensive and meant that 
    method to calculate potential yield based on the pasture utilisation rate no longer worked (overestimated livestock numbers)
"""
# # Load new livestock mapping data from CSIRO, drop some columns, downcast and save a lite version. Only need to do this once then load the lite version
# lmap = pd.read_csv('N:/Data-Master/Profit_map/From_CSIRO/20241114/lmap.csv', low_memory = False).copy()
# lmap = lmap.drop(columns = ['Unnamed: 0', 'ha_dairy', 'ha_pixel', 'no_sheep', 'Beef Cattle', 'Dairy Cattle', 'Sheep', 'heads_mapped_cum', 'SPREAD_colour'])
# downcast(lmap)
# lmap.to_hdf('N:/Data-Master/Profit_map/From_CSIRO/20241114/lmap.h5', key = 'lmap', mode = 'w', format = 't')

# # Load livestock map lite version, column names and descriptions: N:/Data-Master/Profit_map/From_CSIRO/20210910/lmap_variable_names_description.docx
# lmap = pd.read_hdf('N:/Data-Master/Profit_map/From_CSIRO/20241114/lmap.h5')
...
ludf = cell_df.merge(lmap[['X-round', 'Y-round', 'SPREAD_id_mapped', 'kgDMhayr']], how = 'left', on = ['X-round', 'Y-round'])
...
# Check and save intermediary output file
ludf_summary = ludf.groupby(['PRIMARY_V7', 'SECONDARY_V7', 'C18_DESCRIPTION', 'LU_ID', 'LU_DESC', 'IRRIGATION'], observed = True, as_index = False)['X'].count().sort_values(by = ['PRIMARY_V7', 'SECONDARY_V7'])
ludf_summary.rename(columns={'X': 'CELL_COUNT'}, inplace=True)
ludf_summary.to_csv('N:/Data-Master/LUTO_2.0_input_data/Scripts/Intermediate_data_outputs/LUDF_NLUM_land-use_categories.csv')

# # Check NLUM
# nlum = pd.read_csv(r'N:\Data-Master\National_Landuse_Map\NLUM_2010-11_clip.tif.csv').drop(columns = ['Rowid'])
...
# Summarise LU_ID classes (check that there are no inconsistent categories)
...
print('kgDMhayr mean = ', ludf['kgDMhayr'].mean())
print('PASTURE_KG_DM_HA mean = ', ludf['PASTURE_KG_DM_HA'].mean())
print('PASTURE_KG_DM_HA_JAVI mean = ', ludf['PASTURE_KG_DM_HA_JAVI'].mean())
print('PASTURE_KG_DM_HA, kgDMhayr RMS = ', ludf.eval('(PASTURE_KG_DM_HA - kgDMhayr) ** 2').mean() ** .5)
print('PASTURE_KG_DM_HA, kgDMhayr ABS_diff = ', (ludf.PASTURE_KG_DM_HA - ludf.kgDMhayr).abs().mean())
...
# Aggregate feed requirement factor by SA2 for beef, sheep, and dairy - NOTE: THIS IS NOT USED BUT KEPT IN FOR COMPLETENESS
""" feed requirement factor at the SA4 level, calculated as the ratio of total DSEs to allocate (this is the sum of all DSEs of beef, sheep and dairy, 
    from the PFE table) and feed availability. Feed availability = spur * kgDMhayr * irrig_factor and is calculated at the pixel level and aggregated to 
    SA4 (but only considering pasture pixels). The feed_req_factor tells us how much sustainable capacity exists in an SA4 relative to the number of 
    heads AgStats claims exist within it. If feed_req_factor > 1, this means the number of heads is over the maximum established by the safe pasture 
    utilisation rate. [Navarro, J. Marcos Martinez, R. (2021) Estimating long-term profits, fertiliser and pesticide use baselines in Australian 
    agricultural regions. User Guide. CSIRO, Australia. ] """
...
print('Dairy - Mean YIELD_POT_DAIRY = {:.4f} head/ha'.format(ludf_['YIELD_POT_DAIRY'].mean())) 
...
print('Dairy - Mean YIELD_POT_DAIRY - yield_potential_dairy = {:.4f} head/ha'.format((ludf_.YIELD_POT_DAIRY - ludf_.yield_potential_dairy).abs().mean()))
...
print('Dairy - Mean AC_DAIRY - AC_dairy = {:.4f} $/ha'.format((ludf_.AC_DAIRY - ludf_.AC_dairy).abs().mean()))
...
print('Beef - Mean YIELD_POT_BEEF = {:.4f} head/ha'.format(ludf_['YIELD_POT_BEEF'].mean())) 
...
print('Beef - Mean YIELD_POT_BEEF - yield_potential_beef = {:.4f} head/ha'.format((ludf_.YIELD_POT_BEEF - ludf_.yield_potential_beef).abs().mean()))
...
print('Beef - Mean AC_BEEF - AC_beef = {:.4f} $/ha'.format((ludf_.AC_BEEF - ludf_.AC_beef).abs().mean()))
...
print('Sheep - Mean YIELD_POT_SHEEP = {:.4f} head/ha'.format(ludf_['YIELD_POT_SHEEP'].mean())) 
...
print('Sheep - Mean YIELD_POT_SHEEP - yield_potential_sheep = {:.4f} head/ha'.format((ludf_.YIELD_POT_SHEEP - ludf_.yield_potential_sheep).abs().mean()))
...
print('Sheep - Mean AC_SHEEP - AC_sheep = {:.4f} $/ha'.format((ludf_.AC_SHEEP - ludf_.AC_sheep).abs().mean()))
...
ludf_ = ludf_.merge(lmap[['X-round', 'Y-round', 'rev_dairy', 'yield_potential_sheep']], how = 'left', on = ['X-round', 'Y-round'])
