...
############################################################################################################################################
# Add livestock to cell_df commodity mapping
############################################################################################################################################
...
# tmp_df = lmap[['X-round', 'Y-round', 'SPREAD_id_mapped', 'SPREAD_mapped', 'yield_potential_beef', 'yield_potential_sheep', 'yield_potential_dairy']]
tmp_df = lmap.drop(columns = ['X', 'Y', 'SA2_ID'])
ludf = cell_df.merge(tmp_df, how = 'left', left_on = ['X-round', 'Y-round'], right_on = ['X-round', 'Y-round'])
...
ludf['LU_DESC'] = ludf['LU_DESC'].cat.add_categories(['Dairy Cattle', 'Beef Cattle', 'Sheep', 'Plantation forestry'])
ludf.loc[ludf['LU_ID'] == 31, 'LU_DESC'] = 'Dairy Cattle'
ludf.loc[ludf['LU_ID'] == 32, 'LU_DESC'] = 'Beef Cattle'
ludf.loc[ludf['LU_ID'] == 33, 'LU_DESC'] = 'Sheep'

# Set a new LU class for plantation forestry (including hardwood, softwood, agroforestry)
index = ludf.query('SECONDARY_V7 in ["3.1 Plantation forestry", "4.1 Irrigated plantation forestry"]').index
ludf.loc[index, 'LU_ID'] = 27
...
ludf['PASTURE_KG_DM_HA_JAVI'].mean()
ludf.eval('(PASTURE_KG_DM_HA - PASTURE_KG_DM_HA_JAVI) ** 2').mean() ** .5
ludf.eval('PASTURE_KG_DM_HA - PASTURE_KG_DM_HA_JAVI').mean()
ludf[['PASTURE_KG_DM_HA', 'PASTURE_KG_DM_HA_JAVI']].sample(n = 100000).corr() # 0.98 correlation

# Put pasture data side-by-side for visual comparison
lmap[['X-round', 'Y-round', 'kgDMhayr']].merge(ludf[['X-round', 'Y-round', 'PASTURE_KG_DM_HA', 'PASTURE_KG_DM_HA_JAVI']], how = 'left', on = ['X-round', 'Y-round'])
...
lmap['YIELD_POT_BEEF_DRY'] = lmap.eval('safe_pur * kgDMhayr / (8 * 365 * 0.85)')
lmap['AC_BEEF_JN'] = lmap.eval('AC_BEEF * yield_potential_beef')
lmap['AC_BEEF_BB'] = lmap.eval('AC_BEEF * YIELD_POT_BEEF_DRY')
...
lmap[['SPREAD_mapped', 'kgDMhayr', 'PASTURE_KG_DM_HA', 'PASTURE_KG_DM_HA_JAVI', 'YIELD_POT_BEEF_DRY', 'yield_potential_beef', 'safe_pur', 'feed_req_factor', 'head_adjustment_factor', 'AC_beef', 'AC_BEEF_BB', 'AC_BEEF_JN']]
x = lmap.query('(feed_req_factor == 1) & (head_adjustment_factor == 1)')
lmap.loc[x.index, ['SPREAD_mapped', 'kgDMhayr', 'PASTURE_KG_DM_HA', 'PASTURE_KG_DM_HA_JAVI', 'YIELD_POT_BEEF_DRY', 'yield_potential_beef', 'safe_pur', 'feed_req_factor', 'head_adjustment_factor', 'AC_beef', 'AC_BEEF_BB', 'AC_BEEF_JN']]
...
ludf['YIELD_POT_BEEF_DRY'] = ludf.eval('SAFE_PUR * PASTURE_KG_DM_HA / (8 * 365 * 0.85)')
...
ludf['AC_BEEF_BB'] = ludf.eval('AC_BEEF * YIELD_POT_BEEF_DRY')
ludf['FOC_BEEF_BB'] = ludf.eval('FOC_BEEF * YIELD_POT_BEEF_DRY')
ludf['FLC_BEEF_BB'] = ludf.eval('FLC_BEEF * YIELD_POT_BEEF_DRY')
ludf['FDC_BEEF_BB'] = ludf.eval('FDC_BEEF * YIELD_POT_BEEF_DRY')
...
lmap2 = lmap[['X-round', 'Y-round', 'yield_potential_beef', 'yield_potential_sheep', 'yield_potential_dairy']].merge(ludf[['X-round', 'Y-round', 'YIELD_POT_BEEF_DRY', 'YIELD_POT_SHEEP_DRY', 'YIELD_POT_DAIRY_DRY']], how = 'left', on = ['X-round', 'Y-round'])
lmap2[['yield_potential_beef', 'YIELD_POT_BEEF_DRY', 'yield_potential_sheep', 'YIELD_POT_SHEEP_DRY', 'yield_potential_dairy', 'YIELD_POT_DAIRY_DRY']]
lmap2[['yield_potential_beef', 'YIELD_POT_BEEF_DRY']].sample(n = 100000).corr() 
lmap2.eval('(yield_potential_beef - YIELD_POT_BEEF_DRY) ** 2').mean() ** .5
...
lmap2.eval('(yield_potential_beef - YIELD_POT_BEEF_DRY)').mean()
