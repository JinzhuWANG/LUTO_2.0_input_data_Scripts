...
def save_NLUM_to_GeoDataFrame():
    ...
    gdfp = gpd.GeoDataFrame.from_features(list(results), crs = src.crs)
    ...
    SPREAD_dict = {
                    ...
                    2: 'Grazing - woodland and open forest',
    ...
    # Save to file
    NLUM_table.to_csv(r'N:\Planet-A\Data-Master\National_Landuse_Map\NLUM_table.csv')
...
def create_cell_df():
    ...
    dataFlat = NLUM_ID_raster[NLUM_mask].data
    ...
    # Convert rasterio shape features to GeoDataFrame
    rnd_gdf = gpd.GeoDataFrame.from_features(list(results), crs = meta['crs'])
...
def join_SA2_ID():
    ...
    dataFlat = SA2_raster_clipped[NLUM_mask]
...
def add_NLUM_SA2_ID():
    ...
    NLUM_table = pd.read_csv(r'N:\Planet-A\Data-Master\National_Landuse_Map\NLUM_table.csv')
    NLUM_table = NLUM_table[['VALUE', 'SPREAD_ID', 'SPREAD_DESC', 'IRRIGATION']]
...
def join_ABARES():
    ...
    dataFlat = ABARES_raster_clipped[NLUM_mask]
...
def join_LGAs():
    ...
    # Flatten the NRM 2D array to 1D array of valid values only
    dataFlat = LGA_raster_clipped[NLUM_mask]
...
def join_NRM():
    ...
    # Flatten the NRM 2D array to 1D array of valid values only
    dataFlat = NRM_raster_clipped[NLUM_mask]
...
def join_IBRA():
    ...
    # Flatten the NRM 2D array to 1D array of valid values only
    dataFlat = IBRA_raster_clipped[NLUM_mask]
...
def join_BOM_RR():
    ...
    # Flatten the NRM 2D array to 1D array of valid values only
    dataFlat = RR_raster_clipped[NLUM_mask]
...
def join_BOM_DD():
    ...
    # Flatten the NRM 2D array to 1D array of valid values only
    dataFlat = DD_raster_clipped[NLUM_mask]
...
def join_AQUEDUCT():
    ...
    dataFlat = AD_raster_clipped[NLUM_mask]
...
def join_WATER_STRESS_NWC():
    ...
    # Open a new GeoTiFF file
    with rasterio.open(r'N:\Planet-A\Data-Master\Water\Water_stressed_catchments\WATER_STRESS_NWC.tif', 'w+', dtype = 'uint8', nodata = 255, **meta) as dst:
    ...
    cell_df['WATER_STRESS_COD'] = dataFlat
...
def join_NLUM():
    ...
    tmp = pd.read_csv(r'N:\Planet-A\Data-Master\National_Landuse_Map\NLUM_table.csv')
...
def join_POT_IRRIGABLE():
    ...
    # Import shapefile to GeoPandas DataFrame
    gdf = gpd.read_file(r'N:\Planet-A\Data-Master\Water\Irrigation_areas\irrv1ac.shp')
    
    # Access geometry and field to rasterise
    shapes = ((geom, value) for geom, value in zip(gdf.geometry, gdf['IRRV1AC_ID'] > 0))
    
    # Open a new GeoTiFF file
    outfile = r'N:\Planet-A\Data-Master\Water\Irrigation_areas\POTENTIAL_IRRIGATION_AREAS.tif'
    with rasterio.open(outfile, 'w+', dtype = 'uint8', nodata = 255, **meta) as out:
    ...
    # Flatten the SA2 2D array to 1D array of valid values only
    dataFlat = raster_clipped[NLUM_mask]
    
    # Add to cell_df dataframe
    cell_df['POTENTIAL_IRRIGATION_AREAS'] = dataFlat
...
def join_NVIS():
    ...
    #     with rasterio.open(r'N:\Planet-A\Data-Master\NVIS\GRID_NVIS6_0_AUST_EXT_MVG\aus6_0e_mvg.tif', 'w+', dtype='int32', nodata='0', **meta) as dst:
    ...
    # Save as geoTiff
    with rasterio.open(r'N:\Planet-A\Data-Master\NVIS\GRID_NVIS6_0_AUST_EXT_MVG\aus6_0e_mvg.tif', 'w+', nodata='0', dtype='uint8', **meta) as dst:
        dst.write_band(1, NVIS_raster_clipped)
    
    # Flatten 2D array to 1D array of valid values only
    dataFlat = NVIS_raster_clipped[NLUM_mask]
    ...
    # Save as geoTiff
    with rasterio.open(r'N:\Planet-A\Data-Master\NVIS\GRID_NVIS6_0_AUST_EXT_MVS\aus6_0e_mvs.tif', 'w+', nodata='0', dtype='uint8', **meta) as dst:
        dst.write_band(1, NVIS_raster_clipped)
    ...
    # Save as geoTiff
    with rasterio.open(r'N:\Planet-A\Data-Master\NVIS\GRID_NVIS6_0_AUST_PRE_MVG\aus6_0p_mvg.tif', 'w+', nodata='0', dtype='uint8', **meta) as dst:
        dst.write_band(1, NVIS_raster_clipped)
    ...
    # Save as geoTiff
    with rasterio.open(r'N:\Planet-A\Data-Master\NVIS\GRID_NVIS6_0_AUST_PRE_MVS\aus6_0p_mvs.tif', 'w+', nodata='0', dtype='uint8', **meta) as dst:
        dst.write_band(1, NVIS_raster_clipped)
...
join_BOM_DD()
join_AQUEDUCT()
join_WATER_STRESS_NWC()
join_NLUM()
join_POT_IRRIGABLE()
join_NVIS()

cell_df.to_pickle(cell_df_path_pkl)
# cell_df = pd.read_pickle(cell_df_path_pkl)
