...
crops_sum_df[['SA2_ID', 'SA2_Name', 'SPREAD_ID', 'SPREAD_Name', 'Irrigation', 'Yield', 'P1', 'AC', 'QC', 'FDC', 'FOC', 'FLC', 'WR', 'WP', 'Costs_ha_TRUE', 'Costs_t_TRUE']].head(100)
...
# adf = adf.drop(columns = ['SPREAD_ID', 'Irrigation'])
adf[~adf.isna().any(axis = 1)].sort_values(by = 'Yield', ascending = False).head(100)

err = adf[adf.isna().any(axis = 1)].query('(SPREAD_ID >= 5) & (SPREAD_ID <= 25)')
err[err['SPREAD_ID'] == 7]

x = err.query('SPREAD_ID not in [7, 15]').groupby(['SA2_ID', 'SPREAD_DESC'])[['SPREAD_ID']].count()
x[x['SPREAD_ID'] > 0]
...
crops_sum_df.groupby(['SPREAD_Name'], as_index = False).agg(
    ...
    Yld_med = ('Yield', 'median'),
    Yld_mean = ('Yield', 'mean'),
    Yld_05 = ('Yield', lambda x: x.quantile(0.05)),
    Yld_min = ('Yield', 'min'),
    Yld_95 = ('Yield', lambda x: x.quantile(0.95)),
    Yld_max = ('Yield', 'max'))

crops_df.groupby(['SPREAD_Commodity', 'Irr'], as_index = False).agg(
                    Area_dry = ('area_dry_NLUM', 'sum'),
                    Area_irr = ('area_irrig_NLUM', 'sum')
                    )


c = crops_df[['SA2_Name', 'Commodity', 'SPREAD_Commodity', 'SPREAD_ID', 'Prod', 'Area', 'yield_ABS', 'area_dry_NLUM', 'area_irrig_NLUM', 'pct_irrig', 'Irr', 'Prod_factor', 'prod', 'area', 'Yield', 'Q1', 'P1', 'rev', 'cost_pct', 'AC_rel', 'QC_rel', 'FDC_rel', 'FLC_rel', 'FOC_rel', 'WR', 'WP', 'pfe', 'Costs_ha_TRUE']]
c

adf[adf['SPREAD_ID'] == 15].head(100)

cell_df.groupby(['SPREAD_DESC', 'IRRIGATION'], as_index = False).agg(Area = ('CELL_HA', 'sum'))

pd.pivot_table(crops_sum_df, index = 'SPREAD_Name', columns = 'STATE_ID', values = 'Costs_t', aggfunc = 'mean')

crops_sum_df[crops_sum_df['SPREAD_Name'] == 'Vegetables'].groupby(['SPREAD_Name', 'SA2_Name', 'Irrigation'], as_index = False).agg(
    Yield = ('Yield', 'mean'),
    Costs_tonne = ('Costs_t', 'mean'),
    ).sort_values(by=['SPREAD_Name', 'SA2_Name', 'Irrigation'], ascending=True).head(100)

crops_sum_df.groupby(['SPREAD_Name', 'Irrigation'], as_index = False).agg(
    Yield = ('Yield', 'mean'),
    Yield_std = ('Yield', 'std'),
    Costs_t = ('Costs_t', 'mean'),
    Costs_t_std = ('Costs_t', 'std'),
    ).sort_values(by=['SPREAD_Name', 'Irrigation'], ascending=True).head(100)
