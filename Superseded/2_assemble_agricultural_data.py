...
ag_df['Costs_TRUE'] = ag_df.eval('(AC_rel + FDC_rel + FOC_rel + FLC_rel) + (QC_rel * (Production / Area) * Prod_factor) + (WR * WP)')

######## Aggregate the ABS Commodity-level data to SPREAD class

# Define a lambda function to compute the weighted mean
area_weighted_mean = lambda x: np.average(x, weights = ag_df.loc[x.index, 'Area'])  
prod_weighted_mean = lambda x: np.average(x, weights = ag_df.loc[x.index, 'Production'])  

# Aggregate commodity-level to SPREAD-level using weighted mean based on area of Commodity within SA2
ag_df = ag_df[ag_df['SA2_Name'] == 'Goulburn Region']
ag_sum_df = ag_df.groupby(['SA2_ID', 'SPREAD_ID', 'SPREAD_ID_original', 'Irrigation'], as_index = False).agg(
                    SA2_Name = ('SA2_Name', 'first'),
                    SPREAD_Name = ('SPREAD_Commodity', 'first'),
                    Commodity_count = ('Commodity', 'size'),
                    Production = ('Production', 'sum'), 
                    Area = ('Area', 'sum'),
                    Prod_factor = ('Prod_factor', prod_weighted_mean),
                    Yield = ('Yield', prod_weighted_mean),
                    F1 = ('F1', prod_weighted_mean),
                    Q1 = ('Q1', prod_weighted_mean),
                    P1 = ('P1', prod_weighted_mean),
                    F2 = ('F2', prod_weighted_mean),
                    Q2 = ('Q2', prod_weighted_mean),
                    P2 = ('P2', prod_weighted_mean),
                    F3 = ('F3', prod_weighted_mean),
                    Q3 = ('Q3', prod_weighted_mean),
                    P3 = ('P3', prod_weighted_mean),
                    Revenue_TRUE = ('rev', area_weighted_mean),
                    Costs_TRUE = ('Costs_TRUE', area_weighted_mean),
...
ag_sum_df['Costs'] = ag_sum_df.eval('(AC_rel + FDC_rel + FOC_rel + FLC_rel) + (QC_rel * (Production / Area) * Prod_factor) + (WR * WP)')

cols = ['SPREAD_ID', 'SPREAD_ID_original', 'Irrigation', 'SA2_Name', 'SPREAD_Name', 
        'Commodity_count', 'Production', 'Area', 'Prod_factor', 'Yield', 'Revenue_TRUE', 
        'Revenue', 'Costs_TRUE', 'Costs']
ag_sum_df[cols].head(50)

ag_sum_df.drop(columns = ['Costs_PCT_ABS'], inplace = True)

# Join the table to the cell_df dataframe and drop uneccesary columns
adf = cell_df.merge(ag_sum_df, how = 'left', left_on = ['SA2_ID', 'COMMODITIES', 'IRRIGATION'], right_on = ['SA2_ID', 'SPREAD_ID', 'Irrigation']) 
# cell_df = cell_df.drop(columns=['ALBERS_SQM', 'Rowid', 'VALUE', 'COUNT'])

#join the PFE table to NLUM_SA2_gdf and check for NaNs 
NLUM_SA2_gdf2 = NLUM_SA2_gdf.merge(tmp, how='left', left_on=['SA2_ID', 'COMMODITIES', 'IRRIGATION'], right_on=['SA2_ID', 'SPREAD_ID', 'irrigation'])
...
tmp = ag_sum_df.groupby(['SPREAD_Name', 'SA2_ID'])[['Area_crops']].first()
